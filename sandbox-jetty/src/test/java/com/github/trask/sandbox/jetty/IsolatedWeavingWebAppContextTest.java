 ...
 */
public class IsolatedWeavingWebAppContextTest {

    private static final int PORT = 8098;

    private IsolatedWebAppServer server;
    private AsyncHttpClient asyncHttpClient;

    @Before
    public void before() throws Exception {
        server = IsolatedWebAppServer.newWeavingServer();
        server.setPort(PORT);
        server.setDescriptor("src/test/webapp/WEB-INF/web.xml");
        server.setResourceBase("src/test/webapp");
